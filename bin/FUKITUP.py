...
media_info = None
options_list = [None, None, None, None, None, None, None]
...
@click.option('--sample', '-sample')
def click_params(load_media, open_folder, load_preset, info, sox_params, bypass, sample):
    # load_media, open_folder, load_preset, info, sox_params, bypass, sample
    global options_list
    ...
    if sample:
        options_list[6] = sample

    startup(*options_list)

def startup(load_media=None, open_folder=None, load_preset=None, info=None, sox_params=None, bypass=None, sample=None):
    global media_info
    global options_list
    ...
    if sample:
        frame = input('Frame: ')
        # Construct bin\sample.py command
        cmd = [
            'python',
            'bin/sample.py',
            sample,
...
def load_menu_txt():
    ...
    print(
        f"""
        -----------------------------------------------------------------
        | Load Media | Open Folder | Load Preset | Sample | Info | Exit |
          ^            ^             ^    ^        ^        ^      ^
        """
    )
    user_input = input(f'Whatcha wanna do: ').lower()
    ...
    if user_input == 's':
        options_list[6] = dequote(input(f'Media Path: '))
    if user_input == 'e':
        exit()
